...
```mermaid
flowchart LR
    A[Notices a message] --> B[Understands the offer]
    B --> C{Is it relevant and credible?}
    C -->|Yes| D[Compares options]
    C -->|No or unclear| E[Declines or seeks more information]
    D --> F{Chooses freely?}
    F -->|Yes| G[Acts or decides later]
    F -->|No| E
```
